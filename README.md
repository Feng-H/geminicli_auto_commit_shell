...
### 2025-12-24
- **性能优化**: 针对包含大量文件的项目，优化了 `git status` 检查逻辑（使用 `head -n 1`），避免因扫描全部文件导致的卡顿。
- **稳定性增强**: 将 `git add .` 升级为 `git add -A`，确保包括删除在内的所有变更都能被正确暂存。
- **错误处理**: 为 `git init` 和 `git add` 增加了明确的错误检测机制，操作失败时会立即报错并终止流程。
